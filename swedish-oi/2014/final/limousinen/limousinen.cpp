...
struct {
  bool operator()(p2 a, p2 b) const {
    return nabs(a.first) + nabs(a.second) < nabs(b.first) + nabs(b.second);
  }
} customLess;
...
signed main() {
  ...
  while (next_person < n && time + 2 * (nabs(poss[next_person].first) +
                                        nabs(poss[next_person].second)) <=
                                t) {
    time += 2 *
            (nabs(poss[next_person].first) + nabs(poss[next_person].second));
    next_person++;
  }
