...
struct Node {
    ...
    Node(int a) : arr(1), pre(1) {arr[0] = a; pre[0] = a;}
...
p2 query_sum(int l, int r, int V) { // query [l, r)
    ...
    for (l+=N, r+=N; l < r; l/=2, r/=2) {
        if (l&1) res = res + t[l++].sum(V);
        if (r&1) res = res + t[--r].sum(V);
    }
    return res;
}
...
p2 query_count(int l, int r, int V) { // query [l, r)
    p2 res = {0, 0};
    for (l+=N, r+=N; l < r; l/=2, r/=2) {
        if (l&1) res = res + t[l++].count(V);
        if (r&1) res = res + t[--r].count(V);
    }
    return res;
}
