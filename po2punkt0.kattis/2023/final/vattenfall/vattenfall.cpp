...
int main() {
    ...
    while (!q.empty()) {
        ...
        if (vis.find((x << 32) + y) != vis.end()) continue;
        vis.insert((x << 32) + y);
        ...
        if (cliffPoints.find(((x << 32) + y-1)) != cliffPoints.end()) {
            if (0 <= x-1 && x-1 < c && cliffPoints.find((((x-1) << 32) + y)) == cliffPoints.end()) {
                if (abs(x-11) < 2 || dbgf) {
                    dbg("Added", x-1, y);
                }
                q.emplace(x-1, y);
            ...
            if (hi >= 0 && cliffs[x][lo] >= y) hi = -1;
            if (lo == 0 && hi == 0 && cliffs[x][0] < y) lo = 0;
            ...
            if (hi < 0) {
                if (abs(x-11)<2 || dbgf) dbg("Added", x, 0);
                q.emplace(x, 0);
            } else {
                if (abs(x-11)<2 || dbgf) dbg("Added", x, lo+1);
                q.emplace(x, cliffs[x][lo] + 1);
            }
        }
    }

    for (auto& u : vis) dbg(u >> 32, (int)u);
