#include <bits/stdc++.h>

#ifdef DBG
  #include "../../dbg.h"
#else
  #define dbg(...)
#endif

using namespace std;
using ll = long long;

static const ll INF = 1e18;

struct T {
  vector<ll> t;
  vector<ll> la;
  vector<ll> ma, ma2, mac;
  vector<ll> mi, mi2, mic;

  T(int n)
      : t(4 * n),
        la(4 * n),
        ma(4 * n),
        ma2(4 * n),
        mac(4 * n),
        mi(4 * n),
        mi2(4 * n),
        mic(4 * n) {}

  void build(int x, int l, int r, vector<ll>& a) {
    la[x] = 0;
    if (l == r) {
      t[x] = ma[x] = mi[x] = a[l];
      mac[x] = mic[x] = 1;
      ma2[x] = -INF;
      mi2[x] = INF;
      return;
    }

    int mid = (l + r) / 2;
    build(2 * x, l, mid, a);
    build(2 * x + 1, mid + 1, r, a);
    combine(x);
  }

  void combine(int x) {
    t[x] = t[2 * x] + t[2 * x + 1];

    if (ma[2 * x] == ma[2 * x + 1]) {
      ma[x] = ma[2 * x];
      ma2[x] = max(ma2[2 * x], ma2[2 * x + 1]);
      mac[x] = mac[2 * x] + mac[2 * x + 1];
    } else if (ma[2 * x] > ma[2 * x + 1]) {
      ma[x] = ma[2 * x];
      ma2[x] = max(ma2[2 * x], ma[2 * x + 1]);
      mac[x] = mac[2 * x];
    } else {
      ma[x] = ma[2 * x + 1];
      ma2[x] = max(ma[2 * x], ma2[2 * x + 1]);
      mac[x] = mac[2 * x + 1];
    }

    if (mi[2 * x] == mi[2 * x + 1]) {
      mi[x] = mi[2 * x];
      mi2[x] = min(mi2[2 * x], mi2[2 * x + 1]);
      mic[x] = mic[2 * x] + mic[2 * x + 1];
    } else if (mi[2 * x] < mi[2 * x + 1]) {
      mi[x] = mi[2 * x];
      mi2[x] = min(mi2[2 * x], mi[2 * x + 1]);
      mic[x] = mic[2 * x];
    } else {
      mi[x] = mi[2 * x + 1];
      mi2[x] = min(mi[2 * x], mi2[2 * x + 1]);
      mic[x] = mic[2 * x + 1];
    }
  }

  void push(int x, int l, int r) {
    int mid = (l + r) / 2;
    put_add(2 * x, l, mid, la[x]);
    put_add(2 * x + 1, mid + 1, r, la[x]);
    la[x] = 0;

    put_ma(2 * x, ma[x], l == mid);
    put_ma(2 * x + 1, ma[x], mid + 1 == r);

    put_mi(2 * x, mi[x], l == mid);
    put_mi(2 * x + 1, mi[x], mid + 1 == r);
  }

  void put_add(int x, int l, int r, ll v) {
    t[x] += (ll)(r - l + 1) * v;
    la[x] += v;

    ma[x] += v;
    if (ma2[x] != -INF)
      ma2[x] += v;

    mi[x] += v;
    if (mi2[x] != INF)
      mi2[x] += v;
  }

  void put_ma(int x, ll v, bool one) {
    if (v >= ma[x])
      return;
    t[x] -= (ll)ma[x] * mac[x];
    ma[x] = v;
    t[x] += (ll)ma[x] * mac[x];
    if (one)
      mi[x] = ma[x];
    else if (v <= mi[x])
      mi[x] = v;
    else if (v < mi2[x])
      mi2[x] = v;
  }

  void put_mi(int x, ll v, bool one) {
    if (v <= mi[x])
      return;
    t[x] -= mi[x] * mic[x];
    mi[x] = v;
    t[x] += mi[x] * mic[x];
    if (one)
      ma[x] = mi[x];
    else if (v >= ma[x])
      ma[x] = v;
    else if (v > ma2[x])
      ma2[x] = v;
  }

  void chmin(int x, int l, int r, int ql, int qr, ll v) {
    if (l > qr || r < ql || ma[x] <= v)
      return;
    if (l >= ql && r <= qr && v > ma2[x]) {
      put_ma(x, v, l == r);
      return;
    }

    push(x, l, r);
    int mid = (l + r) / 2;
    chmin(2 * x, l, mid, ql, qr, v);
    chmin(2 * x + 1, mid + 1, r, ql, qr, v);
    combine(x);
  }

  void chmax(int x, int l, int r, int ql, int qr, ll v) {
    if (l > qr || r < ql || mi[x] >= v)
      return;
    if (l >= ql && r <= qr && mi2[x] > v) {
      put_mi(x, v, l == r);
      return;
    }

    push(x, l, r);
    int mid = (l + r) / 2;
    chmax(2 * x, l, mid, ql, qr, v);
    chmax(2 * x + 1, mid + 1, r, ql, qr, v);
    combine(x);
  }

  void add(int x, int l, int r, int ql, int qr, ll v) {
    if (l > qr || r < ql)
      return;
    if (l >= ql && r <= qr) {
      put_add(x, l, r, v);
      return;
    }

    push(x, l, r);
    int mid = (l + r) / 2;
    add(2 * x, l, mid, ql, qr, v);
    add(2 * x + 1, mid + 1, r, ql, qr, v);
    combine(x);
  }

  ll query(int x, int l, int r, int ql, int qr) {
    if (l > qr || r < ql)
      return 0;
    if (l >= ql && r <= qr)
      return t[x];

    push(x, l, r);

    int mid = (l + r) / 2;
    return query(2 * x, l, mid, ql, qr) + query(2 * x + 1, mid + 1, r, ql, qr);
  }
};

signed main() {
  cin.tie(0)->sync_with_stdio(0);
  int n, q;
  cin >> n >> q;

  dbg(n, q);

  vector<ll> a(n);
  for (int i = 0; i < n; i++)
    cin >> a[i];

  dbg(a);

  T t(n);
  t.build(1, 0, n - 1, a);
  while (q--) {
    int op, x, y;
    ll z;
    cin >> op >> x >> y;

    dbg(op, x, y);

    if (op == 0) {
      cin >> z;
      t.chmin(1, 0, n - 1, x, y - 1, z);
    } else if (op == 1) {
      cin >> z;
      t.chmax(1, 0, n - 1, x, y - 1, z);
    } else if (op == 2) {
      cin >> z;
      t.add(1, 0, n - 1, x, y - 1, z);
    } else {
      cout << t.query(1, 0, n - 1, x, y - 1) << '\n';
    }
  }
}
