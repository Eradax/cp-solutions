...
{
    template<typename It, typename OutIt, typename ExtractKey>
    static bool ssort(It begin, It end, OutIt buffer_begin, ExtractKey && extract_key)
    {
        bool first_result = RadixSorter<V>::ssort(begin, end, buffer_begin, [&](auto && o)
        {
        ...
        if (first_result)
        {
            return !RadixSorter<K>::ssort(buffer_begin, buffer_begin + (end - begin), begin, extract_first);
        }
        else
        {
            return RadixSorter<K>::ssort(begin, end, buffer_begin, extract_first);
        }
    }
...
struct RadixSorter<const std::pair<K, V> &>
{
    ...
    static bool ssort(It begin, It end, OutIt buffer_begin, ExtractKey && extract_key)
    {
        bool first_result = RadixSorter<V>::ssort(begin, end, buffer_begin, [&](auto && o) -> const V &
        {
            return extract_key(o).second;
        });
...
struct TupleRadixSorter
{
    ...
    template<typename It, typename OutIt, typename ExtractKey>
    static bool ssort(It begin, It end, OutIt out_begin, OutIt out_end, ExtractKey && extract_key)
    {
        bool which = NextSorter::ssort(begin, end, out_begin, out_end, extract_key);
        auto extract_i = [&](auto && o)
        {
            return std::get<I>(extract_key(o));
        };
        if (which)
            return !ThisSorter::ssort(out_begin, out_end, begin, extract_i);
        else
            return ThisSorter::ssort(begin, end, out_begin, extract_i);
    }
...
{
    template<typename It, typename OutIt, typename ExtractKey>
    static bool ssort(It, It, OutIt, OutIt, ExtractKey &&)
    {
        return false;
    }
...
struct RadixSorter<std::tuple<Args...>>
{
    ...
    static bool ssort(It begin, It end, OutIt buffer_begin, ExtractKey && extract_key)
    {
        return SorterImpl::ssort(begin, end, buffer_begin, buffer_begin + (end - begin), extract_key);
    }
...
struct RadixSorter<std::array<T, S>>
{
    ...
    static bool ssort(It begin, It end, OutIt buffer_begin, ExtractKey && extract_key)
    {
        ...
        for (size_t i = S; i > 0; --i)
        {
            ...
            if (which)
                which = !RadixSorter<T>::ssort(buffer_begin, buffer_end, begin, extract_i);
            else
                which = RadixSorter<T>::ssort(begin, end, buffer_begin, extract_i);
        }
        return which;
    }
...
struct FallbackRadixSorter : RadixSorter<decltype(to_radix_sort_key(std::declval<T>()))>
{
    ...
    static bool ssort(It begin, It end, OutIt buffer_begin, ExtractKey && extract_key)
    {
        return base::ssort(begin, end, buffer_begin, [&](auto && a) -> decltype(auto)
        {
...
struct UnsignedInplaceSorter
{
    ...
    template<typename It, typename ExtractKey>
    static void ssort(It begin, It end, std::ptrdiff_t num_elements, ExtractKey & extract_key, void (*next_sort)(It, It, std::ptrdiff_t, ExtractKey &, void *), void * sort_data)
    {
    ...
    static void american_flag_sort(It begin, It end, ExtractKey & extract_key, void (*next_sort)(It, It, std::ptrdiff_t, ExtractKey &, void *), void * sort_data)
    {
        ...
        if (Offset + 1 != NumBytes || next_sort)
        {
            ...
            for (uint8_t * it = remaining_partitions, * it_end = remaining_partitions + num_partitions; it != it_end; ++it)
            {
                ...
                {
                    UnsignedInplaceSorter<StdSortThreshold, AmericanFlagSortThreshold, CurrentSubKey, NumBytes, Offset + 1>::ssort(partition_begin, partition_end, num_elements, extract_key, next_sort, sort_data);
                }
...
{
    template<typename It, typename ExtractKey>
    inline static void ssort(It begin, It end, std::ptrdiff_t num_elements, ExtractKey & extract_key, void (*next_sort)(It, It, std::ptrdiff_t, ExtractKey &, void *), void * next_sort_data)
    {
...
struct ListInplaceSorter
{
    ...
    template<typename It, typename ExtractKey>
    static void ssort(It begin, It end, ExtractKey & extract_key, ListSortData<It, ExtractKey> * sort_data)
    {
        ...
        if (!StdSortIfLessThanThreshold<StdSortThreshold>(end_of_shorter_ones, end, num_elements, extract_key))
        {
            ...
            InplaceSorter<StdSortThreshold, AmericanFlagSortThreshold, ElementSubKey>::ssort(end_of_shorter_ones, end, num_elements, extract_key, sort_next_element, sort_data);
        }
    }
    ...
    static void sort_from_recursion(It begin, It end, std::ptrdiff_t, ExtractKey & extract_key, void * next_sort_data)
    {
        ...
        {
            ssort(begin, end, extract_key, &offset);
        }
    }


    template<typename It, typename ExtractKey>
    static void ssort(It begin, It end, std::ptrdiff_t, ExtractKey & extract_key, void (*next_sort)(It, It, std::ptrdiff_t, ExtractKey &, void *), void * next_sort_data)
    {
...
{
    template<typename It, typename ExtractKey>
    static void ssort(It begin, It end, std::ptrdiff_t, ExtractKey & extract_key, void (*next_sort)(It, It, std::ptrdiff_t, ExtractKey &, void *), void * sort_data)
    {
...
{
    template<typename It, typename ExtractKey>
    static void ssort(It, It, std::ptrdiff_t, ExtractKey &, void *)
    {
    }
};
...
{
    template<typename It, typename ExtractKey>
    static void ssort(It begin, It end, std::ptrdiff_t num_elements, ExtractKey & extract_key, void * next_sort_data = nullptr)
    {
        if (StdSortIfLessThanThreshold<StdSortThreshold>(begin, end, num_elements, extract_key))
            return;

        void (*next_sort)(It, It, std::ptrdiff_t, ExtractKey &, void *) = static_cast<void (*)(It, It, std::ptrdiff_t, ExtractKey &, void *)>(&SortStarter<StdSortThreshold, AmericanFlagSortThreshold, typename CurrentSubKey::next>::ssort);
        if (next_sort == static_cast<void (*)(It, It, std::ptrdiff_t, ExtractKey &, void *)>(&SortStarter<StdSortThreshold, AmericanFlagSortThreshold, SubKey<void>>::ssort))
            next_sort = nullptr;
        InplaceSorter<StdSortThreshold, AmericanFlagSortThreshold, CurrentSubKey>::ssort(begin, end, num_elements, extract_key, next_sort, next_sort_data);
    }
};
...
void inplace_radix_sort(It begin, It end, ExtractKey & extract_key)
{
    ...
    SortStarter<StdSortThreshold, AmericanFlagSortThreshold, SubKey>::ssort(begin, end, end - begin, extract_key);
}
...
template<typename It, typename ExtractKey>
static void ssort(It begin, It end, ExtractKey && extract_key)
{
    detail::inplace_radix_sort<128, 1024>(begin, end, extract_key);
}

template<typename It>
static void ssort(It begin, It end)
{
    ska::ssort(begin, end, detail::IdentityFunctor());
}
...
bool sort_copy(It begin, It end, OutIt buffer_begin, ExtractKey && key)
{
    ...
    {
        ska::ssort(begin, end, key);
        return false;
    }
    else
        return detail::RadixSorter<typename std::result_of<ExtractKey(decltype(*begin))>::type>::ssort(begin, end, buffer_begin, key);
}
